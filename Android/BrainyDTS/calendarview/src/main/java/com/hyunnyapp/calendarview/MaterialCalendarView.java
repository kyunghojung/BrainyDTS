package com.hyunnyapp.calendarview;
...
import com.hyunnyapp.calendarview.format.ArrayWeekDayFormatter;
import com.hyunnyapp.calendarview.format.DateFormatTitleFormatter;
import com.hyunnyapp.calendarview.format.DayFormatter;
import com.hyunnyapp.calendarview.format.MonthArrayTitleFormatter;
import com.hyunnyapp.calendarview.format.TitleFormatter;
import com.hyunnyapp.calendarview.format.WeekDayFormatter;
...
public class MaterialCalendarView extends ViewGroup {
    final String TAG = MaterialCalendarView.class.getCanonicalName();
    /**
    ...
    private int arrowColor = getResources().getColor(R.color.calendar_arrow_color);
        ...
        try {
            ...
            setArrowColor(a.getColor(
                R.styleable.MaterialCalendarView_mcv_arrowColor,
                    getResources().getColor(R.color.calendar_arrow_color)
            ));
            ...
            if (leftMask == null) {
                leftMask = getResources().getDrawable(R.drawable.mcv_action_previous_small);
            }
            ...
            if (rightMask == null) {
                rightMask = getResources().getDrawable(R.drawable.mcv_action_next_small);
            }
    ...
    private void setupChildren() {
        ...
        buttonPast.setImageResource(R.drawable.mcv_action_previous_small);
        ...
        buttonFuture.setImageResource(R.drawable.mcv_action_next_small);
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        if(this.tileSize > 0) {
            ...
            measureTileWidth = this.tileSize;
            measureTileHeight = this.tileSize;
        }
        else if(specWidthMode == MeasureSpec.EXACTLY) {
            if(specHeightMode == MeasureSpec.EXACTLY) {
                //Pick the larger of the two explicit sizes
                //measureTileWidth = Math.max(desiredTileWidth, desiredTileHeight);
                ...
                measureTileWidth = desiredTileWidth;
                measureTileHeight = desiredTileWidth;
            }
        }
        else if(specHeightMode == MeasureSpec.EXACTLY) {
            //Be the height size the user wants
            measureTileWidth = desiredTileHeight;
            measureTileHeight = desiredTileHeight;
        }

        //Uh oh! We need to default to something, quick!
        if(measureTileWidth <= 0) {
            measureTileWidth = measureTileHeight = dpToPx(DEFAULT_TILE_SIZE_DP);
        }

        //Calculate our size based off our measured tile size
        int measuredWidth = measureTileWidth * MonthView.DEFAULT_DAYS_IN_WEEK;
        int measuredHeight = measureTileHeight * viewTileHieght;

        //Put padding back in from when we took it away
        measuredWidth += getPaddingLeft() + getPaddingRight();
        measuredHeight += getPaddingTop() + getPaddingBottom();

        int clampWidth = clampSize(measuredWidth, widthMeasureSpec);
        int clampHeight = clampSize(measuredHeight, heightMeasureSpec);
        ...
        for (int i = 0; i < count; i++) {
            ...
            int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(
                    MonthView.DEFAULT_DAYS_IN_WEEK * measureTileWidth,
                    MeasureSpec.EXACTLY   // UNSPECIFIED, EXACTLY, AT_MOST
            );
