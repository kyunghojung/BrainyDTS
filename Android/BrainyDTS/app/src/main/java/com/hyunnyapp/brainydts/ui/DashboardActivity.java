...
import com.hyunnyapp.brainydts.R;
import com.hyunnyapp.brainydts.ble.BleConnectionManager;
import com.hyunnyapp.brainydts.ble.BleConnectionManagerDelegate;
import com.hyunnyapp.brainydts.ble.BleMultiConnector;
import com.hyunnyapp.calendarview.MaterialCalendarView;
import com.hyunnyapp.brainydts.singleton.BrainyDTS;
import com.hyunnyapp.yahooweathergatter.WeatherInfo;
import com.hyunnyapp.yahooweathergatter.YahooWeather;
import com.hyunnyapp.yahooweathergatter.YahooWeather.SEARCH_MODE;
import com.hyunnyapp.yahooweathergatter.YahooWeather.UNIT;
import com.hyunnyapp.yahooweathergatter.YahooWeatherExceptionListener;
import com.hyunnyapp.yahooweathergatter.YahooWeatherInfoListener;
...
public class DashboardActivity extends ParentActivity implements BleConnectionManagerDelegate {
    ...
    private void addDevice(BluetoothDevice device, int rssi) {
        ...
        if (!deviceFound && device.getName().equals(DEVICE_NAME)) {
    ...
    public void sendData(String deviceName, String data) {
        Log.d(TAG, "sendData: deviceNmae:" + deviceName);
        if(data != null && data.length() != 0 ) {
    ...
    public void connectDevice(String deviceName) {
        Log.d(TAG, "connectDevice: " + deviceName);
    ...
    private void processMessage(final BluetoothDevice device, String message) {
                ...
                if(message.equals(COMMAND_CONNECTED_DEVICE) || message.equals(COMMAND_DISCONNECTED_DEVICE)) {
                    if (message.equals(COMMAND_CONNECTED_DEVICE)) {
                        ...
                        Handler handler = new Handler()
                        {
                            ...
                            {
                                sendData(device.getName(), "UV");   // sendData "Update Values"
                            }
                        };
                        ...
                        {
                            public void handleMessage(Message msg) {
                                connectDevice(device.getName());    // re-connect
                            }
                        };
                        handler.sendEmptyMessageDelayed(0, 5000);
